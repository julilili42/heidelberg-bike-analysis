import os
import polars as pl
from data_io.formats.formats import ACCIDENT_FORMAT, WEATHER_FORMAT, BICYCLE_FORMAT
...
class DataLoader:
    ...
    def _load_weather(self):
        ...
        files = sorted(
            f for f in os.listdir(self.weather_folder)
            if f.endswith(".csv")
        )
    ...
    def get_bicycle(self, station_name, interval=None, sample_rate=None):
        ...
        bd = bd.drop([
            ...
            "counter_site_id","counter_site","counter_serial",
            "timezone","iso_timestamp","longitude","latitude"
        ])
    ...
    def _load_accidents(self):
        ...
        files = sorted(
            f for f in os.listdir(self.accident_folder)
            if f.endswith(".csv")
        )

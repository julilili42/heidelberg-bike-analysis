...
Low-condition days ($L$) serve as baseline, and relative changes in mean daily bicycle counts are computed as
\[
...
\begin{table}[h!]
\centering
\caption{Weather class definitions used for the event-based analysis.}
\label{tab:weather_classes}
\small
\begin{tabularx}{\linewidth}{lcc}
\toprule
\textbf{Class} & \textbf{Temperature} & \textbf{Precipitation} \\
\midrule
$L$ & $T_{\max} < 10^\circ\mathrm{C}$ & $P_{\text{day}} = 0~\mathrm{mm}$ \\
$M$ & $10 \le T_{\max} < 20^\circ\mathrm{C}$ & $0 < P_{\text{day}} \le 5~\mathrm{mm}$ \\
$H$ & $T_{\max} \ge 20^\circ\mathrm{C}$ & $P_{\text{day}} > 5~\mathrm{mm}$ \\
\bottomrule
\end{tabularx}
\end{table}
...
\caption{Effect of public holidays on station usage across usage types.}
\small
...
\midrule
Mixed        & 1648 & 953  & $-28.6\%$ \\
Recreational & 1260 & 728  & $-42.2\%$ \\
Utilitarian  & 3238 & 2043 & $-42.8\%$ \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[h!]
    \centering
    \caption{todo}
    \includegraphics[width=\columnwidth]{img/hourly_index.png}
    \label{fig:hourly_index}
\end{figure}

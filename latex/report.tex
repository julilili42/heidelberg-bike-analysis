...
\usepackage{bbm} % for better indicator function
...
Finally, we investigate the influence of external factors, such as weather and public holidays, which varies substantially across different usage types. \footnote{Source files are publicly available in the \href{https://github.com/julilili42/heidelberg-bike-analysis}{\textcolor{TUred}{project repository}}.}
...
Sustainable urban traffic relies heavily on the expansion of cycling, which is explicitly promoted by the city of Heidelberg as part of their long-term mobility strategies \cite{heidelberg_radstrategie_2030}. 
To translate these strategies into infrastructure, planners require an understanding of when and why people cycle, not just how many.

Trip purpose is commonly inferred from spatiotemporal bicycle counts; however, existing rule-based approaches struggle to capture hybrid usage patterns \cite{american_bike_patterns}.
...
To address these questions, we analyze hourly bicycle counts from 15 stations in Heidelberg (Figure~\ref{fig:station_map}) using a data-driven approach.
...
Data quality was assessed by counting missing hourly data points. Station failures always resulted in extended periods of missing data rather than continuous zero values. Table~\ref{tab:station_quality} summarizes data availability and basic traffic statistics.
...
\toprule
\textbf{ID} & \textbf{Station} & \textbf{Mean} & \textbf{Avail.} \\
 & & \small{[bikes/d]} & \small{[\%]} \\
\midrule
S1 & Plöck & 4290 & 91.23 \\
...
Public holidays for Baden-Württemberg were obtained from the Mehr Schulferien API \cite{mehr_schulferien}. 
...
we map each station into a low-dimensional feature space. This allows us to use unsupervised k-means clustering \citep{kmeans} to discover different usage types. 
...
\begin{itemize}[noitemsep, topsep=1pt]
    ...
    \item \emph{Recreational (Rec)}: Traffic lacking typical 9-to-5 commuting peaks. While often associated with leisure, in a university city like Heidelberg, this pattern also captures irregular student traffic (e.g., to libraries or lecture halls).
...
Although the maximum silhouette score is achieved at $k=2$ ($0.49$), 
$k=3$ introduces an additional stable cluster (silhouette $0.41$) while preserving a clear elbow in the inertia curve.
...
Features are derived from station-level normalised hourly, daily, and monthly traffic indices $I_h, I_w, I_m$ \cite{american_bike_patterns}, and are conceptually aligned with the indicators used therein. The feature design is motivated by typical bicycle traffic patterns (see Figure~\ref{fig:patterns}). 
Feature relationships were explored using correlation analysis and PCA as sanity checks.
...
\[
P(u \mid s) = \frac{1}{N_s} \sum_{i=1}^{N_s} \mathbbm{1}\bigl( c_i(s) = u \bigr),
\]
...
Clusters with higher scores are labeled
\emph{utilitarian}, intermediate scores as \emph{mixed}, and lower scores as \emph{recreational}.
As centroid ordering relies on the same feature space as clustering, the labels provide descriptive characterisations rather than external validation.
...
\begin{table}[t]
\centering
\caption{Temperature and precipitation classes, defined independently and applied only to the respective analysis.}
...
$q_{33}=10.7^\circ$C, $q_{66}=19.8^\circ$C, $q_{80}=7.1$ mm.
}
\end{table}
...
In the following analysis, each station is assigned to its dominant usage type, defined as the class with the highest assignment probability. 
Figure~\ref{fig:station_map} visualizes the spatial distribution of stations by dominant usage type.

\begin{figure}[t]
    \centering
    \caption{
    Spatial distribution of bicycle counting stations. Marker color and shape indicate the dominant usage type. Marker size reflects the assignment probability of the dominant class.}
...
(i) the analysis is limited to 15 stations within a single city,
(ii) directional flow information is not considered,
(iii) usage types are inferred from temporal patterns rather than ground-truth trip purposes,
and (iv) the choice of $k$ and features favors interpretability over clustering optimality.


\begin{figure}[!htbp]
    \centering
    \caption{Typical hourly traffic patterns by station type on weekdays and weekends. Adapted from \cite{american_bike_patterns}.}
    \includegraphics[width=\columnwidth]{img/figure_patterns.png}
    \label{fig:patterns}
\end{figure}
